...
public class HotelPicServiceImpl {
	...
	public void saveHotelPic(Long hotelId, Long roomTypeId, String picType, String url, String updateBy) {
		...
		if (hotelId == null) {
			throw MyErrorEnum.HOTEL_ID_IS_NULL.getMyException();
		}
		if (StringUtils.isBlank(picType)) {
			throw MyErrorEnum.HOTEL_PIC_TYPE_IS_NULL.getMyException();
		}
		...
		if (hotelPicTypeEnum == null) {
			throw MyErrorEnum.HOTEL_PIC_TYPE_ERROR.getMyException();
		}
		if (hotelPicTypeEnum.getCode() == HotelPicTypeEnum.roomType.getCode() && roomTypeId == null) {
			throw MyErrorEnum.HOTEL_NEED_ROOM_TYPE_ID.getMyException();
		}

		if (roomTypeId != null) {
			roomType = roomTypeMapper.selectByPrimaryKey(roomTypeId);
			if (roomType == null) {
				throw MyErrorEnum.HOTEL_ROOM_TYPE_ID_ERROR.getMyException();
			}
		}

		Hotel hotel = hotelMapper.selectByPrimaryKey(hotelId);
		if (hotel == null || hotel.getId() == null) {
			throw MyErrorEnum.HOTEL_ID_NOT_EXIST.getMyException();
		}
		if (StringUtils.isBlank(url)) {
			throw MyErrorEnum.HOTEL_PIC_ADDRESS_ERROR.getMyException();
		}
	...
	public void delPicture(Long hotelId, Long roomTypeId, String picType, String url, String updateBy) {
		...
		if (hotelPicTypeEnum.getCode() == HotelPicTypeEnum.roomType.getCode() && roomTypeId == null) {
			throw MyErrorEnum.HOTEL_NEED_ROOM_TYPE_ID_DELETE.getMyException();
		} else {
			if (roomTypeId != null) {
				roomType = roomTypeMapper.selectByPrimaryKey(roomTypeId);
			}
			if (roomType == null) {
				throw MyErrorEnum.HOTEL_ROOM_TYPE_ID_ERROR_DELETE.getMyException();
			}
		}
	...
	public void updatePicResourceListToInValid(List<PicResource> picResourceList) {
		if (CollectionUtils.isEmpty(picResourceList)) {
			throw MyErrorEnum.HOTEL_PARAM_ERROR_UPDATE_PIC.getMyException();
		}
		for (PicResource p : picResourceList) {
			updatePicResourceToInValid(p);
		}
